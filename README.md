...
Synthetic transactions emitted by the scenario generator and processed by the baseline. An injected positive label describes the fixture, not real customer fraud. This is a test artifact, not production performance.

[Open the result record](docs/output-example.json) · [Open the HTML report](docs/output-showcase.html)

Reproduce the aggregate replay below. The CLI reports all 200 sequences; the showcased first sequence is generated by the same `generate_sequences` function with seed 17. Its 1/1 detection is a single-example result: the full run detects **86/200 injected positives (43% recall)**. The baseline is a deterministic amount/channel rule, not a trained fraud classifier.

```sh
python -m edgecase_foundry.cli --hypothesis '3 small purchases under $5 then an online purchase over $700 in a new city within 20 minutes' --count 200 --seed 17
```
